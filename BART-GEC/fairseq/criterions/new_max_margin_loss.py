...
def max_margin_loss(model, pos_ori_lprobs, pos_ori_target, n_list, ori_sample, sample_size, margin, ignore_index=None):
    '''
    ...
    for i in range(max(n_list)):
        ...
        tmp_sample['net_input']['prev_output_tokens'] = ori_sample['net_input']['prev_output_tokens_neg'][i]
    ...
    for sent_id in range(a):
        for candi in range(n_list[sent_id]):
            local_mml = triplet_loss(-zero_anchor[sent_id], -final_new_nll_other_matrix[candi][sent_id], -new_nll_matrix[sent_id])/(n_list[sent_id])
            final_mml += local_mml
    return final_mml


def mml_label_smoothed_nll_loss(model, pos_lprobs, pos_target, pos_ori_lprobs, pos_ori_target, n_list, tmp_sample,
    ...
    mml = max_margin_loss(model, pos_ori_lprobs, pos_ori_target, n_list, tmp_sample, sample_size, margin, ignore_index)
...
@register_criterion('new_max_margin_loss')
class New_Max_Margin_Criterion(FairseqCriterion):
    ...
    def forward(self, model, sample, valid=False, reduce=True):
        ...
        if 'valid' in sample.keys():
            ...
            import copy
            # print("Keys in sample: {}".format(sample.keys()))
            ...
            loss, nll_loss = self.mml_compute_loss(model, n_list, pos_net_output, tmp_sample, pos_sample,
    ...
    def mml_compute_loss(self, model, n_list, pos_net_output, tmp_sample, pos_sample, sample_size, reduce=True):
        ...
        loss, nll_loss = mml_label_smoothed_nll_loss(model, pos_lprobs, pos_target, pos_ori_lprobs, pos_ori_target,
                                                     n_list, tmp_sample, sample_size, self.margin,
